...
ok(saw(lines, "seal answers the jars"), "the plinth pays the seal")
ok(not g.won, "the seal opens the stair, not the game")
ok(g.player.location.connections["up"] ~= nil, "the way up stands open")
-- the sphere: steel fails, the word does not
g:doCommand("go up")
ok(saw(lines, "nothing in it obeys the ground"), "the sphere receives you")
g:doCommand("pry coffin")
ok(saw(lines, "tenant UNCOILS"), "the coffin gives up its tenant")
g:doCommand("attack fungal horror")
ok(saw(lines, "Steel is a treadmill"), "the blade cannot end it")
g:doCommand("read prayers")
ok(saw(lines, "PRAYER OF WRATH"), "the walls teach the words")
sug = g:suggestions()
ok(has(sug.nouns, "prayer of wrath"), "read prayers puts the words on the wheel")
g:doCommand("say prayer of balm")
g:doCommand("say prayer of wrath")
ok(saw(lines, "unwritten limb by limb"), "the chamber's law ends it")
ok(g.won, "the tomb is quiet: won")
ok(g.score == 45, "this run banks 45 of 50 (Silas went unmet)")
...
g:doCommand("pry coffin")
g:doCommand("read prayers")
local wBefore = g.wounds
g:doCommand("say prayer of balm")
ok(g.wounds == wBefore - 1, "the balm closes the venom wound")
g:doCommand("say prayer of wrath")
ok(g.won, "the tomb is quiet")
