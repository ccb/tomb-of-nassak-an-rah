...
(() => {
  ...
  // the four burial cylinders, phone layout -- shared by the cyl-*-m twins.
  // cfg = { key, header, kicker, burst:[NAMES], footer }
  function cylsCardM(svg, cfg) {
    ...
    if (cfg.kicker) {                              // class label as a left subtitle,
      const k = label(svg, 20, 64, 13, PH_DIM);   // not right-anchored (long headers
      k.textContent = cfg.kicker;                 // would collide with it)
    }
    const gel = stipple(svg, "dots-" + cfg.key, PH_DIM, 1.4);
    const HUE = { CERULEAN: PH, AMBER: "#ffd06a", VIRIDIAN: "#4ee0a8", ORANGE: FUNGUS };
    const CX = { CERULEAN: 110, AMBER: 253, VIRIDIAN: 393, ORANGE: 536 };
    const burst = cfg.burst || [], bubbles = [], spores = [];
    ["CERULEAN", "AMBER", "VIRIDIAN", "ORANGE"].forEach((name, i) => {
      const cx = CX[name], hue = HUE[name], down = burst.includes(name);
      const put = (nn, a) => el(svg, nn, a);
      if (!down) {
        put("rect", { x: cx - 34, y: 150, width: 68, height: 250, fill: gel });
        put("line", { x1: cx - 36, y1: 142, x2: cx - 36, y2: 400, stroke: PH, "stroke-width": 2 });
        put("line", { x1: cx + 36, y1: 142, x2: cx + 36, y2: 400, stroke: PH, "stroke-width": 2 });
        put("ellipse", { cx, cy: 142, rx: 36, ry: 10, fill: "none", stroke: PH_BRIGHT, "stroke-width": 2 });
        put("ellipse", { cx, cy: 400, rx: 36, ry: 10, fill: "none", stroke: PH });
        put("circle", { cx, cy: 196, r: 9, fill: "none", stroke: PH_DIM, "stroke-width": 1.6 });
        put("polyline", { fill: "none", stroke: PH_DIM, "stroke-width": 1.6, points:
          `${cx - 12},212 ${cx + 12},212 ${cx + 8},300 ${cx + 6},368 ${cx - 6},368 ${cx - 8},300 ${cx - 12},212` });
        if (name === "CERULEAN")
          put("line", { x1: cx + 16, y1: 224, x2: cx + 16, y2: 300, stroke: hue, "stroke-width": 2.4 });
        if (name === "AMBER")
          put("circle", { cx, cy: 236, r: 8, fill: "none", stroke: hue, "stroke-width": 2 });
        if (name === "VIRIDIAN") {
          put("rect", { x: cx - 13, y: 372, width: 11, height: 9, fill: "none", stroke: hue, "stroke-width": 2 });
          put("rect", { x: cx + 2, y: 372, width: 11, height: 9, fill: "none", stroke: hue, "stroke-width": 2 });
        }
        if (name === "ORANGE") {
          put("line", { x1: cx, y1: 300, x2: cx, y2: 250, stroke: hue, "stroke-width": 2.4 });
          for (let s = 0; s < 6; s++)
            spores.push({ q: put("circle", { r: 2.2, fill: FUNGUS }), x: cx + (s - 2.5) * 10, ph: s * 23 });
        }
        for (let b = 0; b < 3; b++)
          bubbles.push({ q: put("circle", { cx: cx + (b - 1) * 12, cy: 398, r: 2.4,
            fill: "none", stroke: hue, opacity: .8 }), ph: i * 31 + b * 57 });
      } else {
        put("polyline", { fill: "none", stroke: PH, "stroke-width": 2, points:
          `${cx - 36},400 ${cx - 36},300 ${cx - 22},322 ${cx - 8},292 ${cx + 6},318 ` +
          `${cx + 20},290 ${cx + 34},320 ${cx + 36},300 ${cx + 36},400` });
        put("ellipse", { cx, cy: 400, rx: 36, ry: 10, fill: "none", stroke: PH });
        put("ellipse", { cx, cy: 422, rx: 58, ry: 10, fill: hue, opacity: .3 });
        put("polyline", { fill: "none", stroke: PH_DIM, "stroke-width": 2,
          points: `${cx - 22},386 ${cx - 6},374 ${cx + 12},386 ${cx + 22},378` });
      }
      const cap = label(svg, cx, 446, 15, down ? hue : PH_DIM);
      cap.setAttribute("text-anchor", "middle"); cap.textContent = name;
    });
    const fw = cfg.footer, cut = fw.length > 34 ? fw.lastIndexOf(" ", 34) : -1;
    const fL1 = cut > 0 ? fw.slice(0, cut) : fw, fL2 = cut > 0 ? fw.slice(cut + 1) : "";
    const foot1 = label(svg, 320, 636, 22, FUNGUS); foot1.setAttribute("text-anchor", "middle");
    const foot2 = label(svg, 320, 668, 22, FUNGUS); foot2.setAttribute("text-anchor", "middle");
    const doWipe = wipe(svg, 640, 720, 2, 12);
    clock(t => {
      const T = t % 200;
      doWipe(T);
      typeOn(hdr, cfg.header, T, 4, 1.2);
      bubbles.forEach(b => b.q.setAttribute("cy", 398 - ((t * 3 + b.ph) % 246)));
      spores.forEach(s => {
        const k = (t * 2 + s.ph) % 130;
        s.q.setAttribute("cx", s.x + Math.sin((t + s.ph) * .25) * 8);
        s.q.setAttribute("cy", 250 - k * 1.2);
        s.q.setAttribute("opacity", k > 108 ? 0 : .5);
      });
      typeOn(foot1, fL1, T, 120, 1.6);
      typeOn(foot2, fL2, T, 120 + fL1.length, 1.6);
    });
  }

  // a canopic-jar close-up, phone layout -- shared by the jar-*-m twins.
  // cfg = { key, head, header, callouts:[...], footer }
  function jarCardM(svg, cfg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 24, PH_BRIGHT);
    const GEL = stipple(svg, "dots-" + cfg.key, PH_DIM, 1.4);
    // the jar body: neck, shoulders, tapering body, rounded base
    el(svg, "path", { fill: GEL, stroke: PH, "stroke-width": 2.2, d:
      "M 250 196 Q 230 210 228 250 L 236 404 Q 240 432 320 434 " +
      "Q 400 432 404 404 L 412 250 Q 410 210 390 196 Z" });
    el(svg, "ellipse", { cx: 320, cy: 196, rx: 70, ry: 12, fill: BG, stroke: PH, "stroke-width": 2 });
    // the stopper: the animal head glyph, scaled up, seated on the neck
    const g = el(svg, "g", { transform: "translate(-448 -505) scale(2.4)" });
    headGlyph(g, cfg.head, 320);
    // the gel contents -- a coiled thing -- and bubbles rising through it
    const coil = el(svg, "path", { fill: "none", stroke: PH, "stroke-width": 1.6, opacity: .7,
      d: "M 292 358 Q 320 328 348 354 Q 356 384 320 388 Q 286 384 300 358" });
    const bubs = [284, 320, 354].map(x => el(svg, "circle", { cx: x, cy: 400, r: 3,
      fill: "none", stroke: PH, opacity: .7 }));
    const calls = (cfg.callouts || []).map((c, i) => label(svg, 40, 486 + i * 36, 18, PH));
    const fw = cfg.footer, cut = fw.length > 34 ? fw.lastIndexOf(" ", 34) : -1;
    const fL1 = cut > 0 ? fw.slice(0, cut) : fw, fL2 = cut > 0 ? fw.slice(cut + 1) : "";
    const foot1 = label(svg, 320, 648, 21, FUNGUS); foot1.setAttribute("text-anchor", "middle");
    const foot2 = label(svg, 320, 678, 21, FUNGUS); foot2.setAttribute("text-anchor", "middle");
    const doWipe = wipe(svg, 640, 720, 2, 12);
    clock(t => {
      const T = t % 210;
      doWipe(T);
      typeOn(hdr, cfg.header, T, 4, 1.1);
      bubs.forEach((b, i) => b.setAttribute("cy", 400 - ((t * 2 + i * 40) % 200)));
      coil.setAttribute("opacity", .5 + .3 * Math.abs(Math.sin(t * .1)));
      (cfg.callouts || []).forEach((c, i) => typeOn(calls[i], c, T, 30 + i * 18, 1.5));
      typeOn(foot1, fL1, T, 150, 1.6);
      typeOn(foot2, fL2, T, 150 + fL1.length, 1.6);
    });
  }

  // the fungal chimney-shaft on its side, phone layout -- shared by the
  // centipede / chimney-*-m twins. cfg = { key, header, charred, centipede, gauge, footer }
  function chimCardM(svg, cfg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 26, PH_BRIGHT);
    const RDOTS = stipple(svg, "dots-" + cfg.key, PH_DIM, 1.1);
    const TCOL = cfg.charred ? "#7a4a24" : FUNGUS;
    // the rock above and below, the shaft between
    el(svg, "rect", { x: 0, y: 150, width: 640, height: 72, fill: RDOTS, opacity: .5 });
    el(svg, "rect", { x: 0, y: 428, width: 640, height: 72, fill: RDOTS, opacity: .5 });
    el(svg, "line", { x1: 0, y1: 222, x2: 640, y2: 222, stroke: PH, "stroke-width": 2 });
    el(svg, "line", { x1: 0, y1: 428, x2: 640, y2: 428, stroke: PH, "stroke-width": 2 });
    // the prayer-glow at the far end
    el(svg, "circle", { cx: 596, cy: 325, r: 46, fill: "none", stroke: PH_BRIGHT, opacity: .4 });
    el(svg, "circle", { cx: 596, cy: 325, r: 26, fill: "none", stroke: PH_BRIGHT, opacity: .65 });
    const tends = [], motes = [];
    for (let i = 0; i < 7; i++) {
      const x = 60 + i * 78;
      tends.push({ q: el(svg, "polyline", { fill: "none", stroke: TCOL, "stroke-width": 2.4 }), x });
      if (!cfg.charred) motes.push({ q: el(svg, "circle", { r: 2.4, fill: FUNGUS }), x, ph: i * 30 });
    }
    let cent = null;
    if (cfg.centipede) {
      cent = el(svg, "g", {});
      for (let s = 0; s < 12; s++)
        el(cent, "polygon", { fill: "none", stroke: PH_BRIGHT, "stroke-width": 1.6, opacity: .85,
          points: `${s * 22 - 8},325 ${s * 22},312 ${s * 22 + 8},325 ${s * 22},338` });
    }
    let cells = null;
    if (cfg.gauge) {
      label(svg, 40, 556, 20, PH_DIM).textContent = cfg.gauge;
      cells = Array.from({ length: 10 }, (_, i) => el(svg, "rect", { x: 250 + i * 34, y: 540,
        width: 26, height: 20, fill: PH, "fill-opacity": .8 }));
    }
    const fw = cfg.footer, cut = fw.length > 32 ? fw.lastIndexOf(" ", 32) : -1;
    const fL1 = cut > 0 ? fw.slice(0, cut) : fw, fL2 = cut > 0 ? fw.slice(cut + 1) : "";
    const foot1 = label(svg, 320, 636, 22, FUNGUS); foot1.setAttribute("text-anchor", "middle");
    const foot2 = label(svg, 320, 668, 22, FUNGUS); foot2.setAttribute("text-anchor", "middle");
    const doWipe = wipe(svg, 640, 720, 2, 12);
    clock(t => {
      ...
      tends.forEach((td, i) => {
        const pts = [];
        for (let q = 0; q <= 5; q++) {
          const sway = cfg.charred ? Math.sin(t * .05 + i) * 2 : Math.sin(t * .12 + q + i) * 8;
          pts.push([td.x + sway, 222 + q * 41.2]);
        }
        td.q.setAttribute("points", pts.map(p => p.join(",")).join(" "));
      });
      motes.forEach(m => {
        const k = (t * 2 + m.ph) % 90;
        m.q.setAttribute("cx", m.x + Math.sin((t + m.ph) * .2) * 6);
        m.q.setAttribute("cy", 428 - k * 2.3); m.q.setAttribute("opacity", k > 76 ? 0 : .5);
      });
      if (cent) {
        cent.setAttribute("transform", `translate(${((t * 26) % 900) - 260} 0)`);
        cent.setAttribute("opacity", t % 40 < 6 ? .2 : 1);
      }
      if (cells) {
        const on = 3 + Math.floor(3 * (1 + Math.sin(t * .1)));
        cells.forEach((r, i) => r.setAttribute("fill-opacity", i < on ? .8 : .1));
      }
  ...
  // the Autarch's carved face, phone layout -- shared by autarch/-c/-e-m.
  // cfg = { key, header, state:'rest'|'skull'|'fire', footers:[...], gauge }
  function autarchCardM(svg, cfg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 25, PH_BRIGHT);
    const SDOTS = stipple(svg, "dots-" + cfg.key, PH, 1);
    const skull = cfg.state !== "rest", fire = cfg.state === "fire";
    const CX = 320, CY = 300, ROPE = fire ? "#7a4a24" : FUNGUS;
    // the spiked halo behind
    const spikes = [];
    for (let i = 0; i < 20; i++) {
      const a = i * Math.PI / 10;
      spikes.push(el(svg, "polygon", { fill: PH, "fill-opacity": .25, stroke: PH,
        "stroke-width": 1, points:
        `${CX + Math.cos(a - .06) * 150},${CY + Math.sin(a - .06) * 150} ` +
        `${CX + Math.cos(a + .03) * (185 + (i % 2) * 22)},${CY + Math.sin(a + .03) * (185 + (i % 2) * 22)} ` +
        `${CX + Math.cos(a + .12) * 150},${CY + Math.sin(a + .12) * 150}` }));
    }
    el(svg, "circle", { cx: CX, cy: CY, r: 150, fill: "none", stroke: PH_DIM });
    // the face: a rounded stone/bone mass, knit cap, court-curl beard
    el(svg, "path", { fill: SDOTS, stroke: PH, "stroke-width": 2.4, d:
      "M 236 250 Q 236 168 320 164 Q 404 168 404 250 L 398 350 " +
      "Q 388 402 320 406 Q 252 402 242 350 Z" });
    // the eyes
    if (fire) {
      [284, 356].forEach(x => {
        el(svg, "ellipse", { cx: x, cy: 286, rx: 20, ry: 15, fill: BG, stroke: PH, "stroke-width": 1.8 });
        el(svg, "circle", { cx: x, cy: 286, r: 8, fill: "#ffd76a", "fill-opacity": .85 });
      });
    } else if (skull) {
      [284, 356].forEach(x => {
        el(svg, "ellipse", { cx: x, cy: 286, rx: 20, ry: 16, fill: BG, stroke: PH, "stroke-width": 1.8 });
        el(svg, "circle", { cx: x, cy: 286, r: 9, fill: FUNGUS, "fill-opacity": .8 });
      });
    } else {
      [284, 356].forEach(x =>
        el(svg, "path", { d: `M ${x - 18} 286 Q ${x} 296 ${x + 18} 286`, fill: "none",
          stroke: PH, "stroke-width": 2 }));
    }
    // nose + mouth
    el(svg, "path", { d: "M 320 300 L 312 330 Q 320 338 328 330 Z", fill: BG, stroke: PH, "stroke-width": 1.6 });
    if (skull) {                                            // a grin, one tooth gone
      el(svg, "line", { x1: 292, y1: 352, x2: 348, y2: 352, stroke: PH, "stroke-width": 1.8 });
      for (let x = 294; x <= 346; x += 8)
        if (x !== 318) el(svg, "line", { x1: x, y1: 352, x2: x, y2: 366, stroke: PH });
    } else {
      el(svg, "path", { d: "M 296 352 Q 320 360 344 352", fill: "none", stroke: PH, "stroke-width": 1.8 });
    }
    // the beard curls
    for (let i = 0; i < 7; i++)
      el(svg, "circle", { cx: 268 + i * 14, cy: 392 + (i % 2) * 6, r: 9, fill: "none",
        stroke: PH_DIM, "stroke-width": 1.4 });
    // the fungal ropes / fire
    const ropes = [];
    if (skull) {
      const ANCH = [[236, 250], [404, 250], [280, 176], [360, 176], [320, 410]];
      ANCH.forEach(([ax, ay], i) => ropes.push({ q: el(svg, "polyline",
        { fill: "none", stroke: ROPE, "stroke-width": 2.4 }), ax, ay, i }));
    }
    const embers = fire ? Array.from({ length: 14 }, (_, i) =>
      ({ q: el(svg, "circle", { r: 2, fill: "#ffd76a" }), x: 200 + i * 34, ph: i * 17 })) : [];
    let cells = null;
    if (cfg.gauge) {
      label(svg, 40, 556, 20, PH_DIM).textContent = cfg.gauge;
      cells = Array.from({ length: 10 }, (_, i) => el(svg, "rect", { x: 190 + i * 36, y: 540,
        width: 26, height: 20, fill: FUNGUS, "fill-opacity": .8 }));
    }
    const foots = (cfg.footers || []).filter(f => f !== cfg.gauge)
      .map((f, i) => { const n = label(svg, 320, 616 + i * 32, 21, FUNGUS);
    ...
    clock(t => {
      ...
      doWipe(T);
      typeOn(hdr, cfg.header, T, 4, 1.2);
      const breath = 1 + Math.sin(t * .06) * .02;
      spikes.forEach(s => s.setAttribute("opacity", fire ? .3 + Math.abs(Math.sin(t * .3)) * .5 : .5 + Math.sin(t * .06) * .2));
      ropes.forEach(r => {
        const pts = [[r.ax, r.ay]];
        for (let q = 1; q <= 4; q++) {
          const sway = (fire ? 1 : Math.sin(t * .12 + q + r.i) * 6);
          pts.push([r.ax + (r.ax - CX) * .12 * q + sway, r.ay + (r.ay - CY) * .12 * q]);
        }
        r.q.setAttribute("points", pts.map(p => p.join(",")).join(" "));
      });
      embers.forEach(e => { const k = (t * 3 + e.ph) % 120;
        e.q.setAttribute("cx", e.x + Math.sin((t + e.ph) * .2) * 10);
        e.q.setAttribute("cy", 420 - k * 2.6); e.q.setAttribute("opacity", k > 100 ? 0 : .8); });
      if (cells) { const on = Math.max(0, 10 - Math.floor(T / 20));
        cells.forEach((r, i) => r.setAttribute("fill-opacity", i < on ? .8 : .1)); }
      foots.forEach((ft, i) => typeOn(ft.n, ft.f, T, 120 + i * 20, 1.6));
    });
  }

  // the zero-g burial sphere, phone layout -- shared by sphere-b/d/e/f-m.
  // cfg = { key, header, state:'upright'|'reforged'|'shatter-vacant'|'shatter-tenant', gauge, callouts, footer }
  function sphereCardM(svg, cfg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 22, PH_BRIGHT);
    const FD = stipple(svg, "dots-" + cfg.key, PH_DIM, 1.1);
    const CX = 320, CY = 292;
    const shattered = cfg.state.indexOf("shatter") === 0;
    const tenant = cfg.state === "upright" || cfg.state === "shatter-tenant";
    el(svg, "circle", { cx: CX, cy: CY, r: 210, fill: "none", stroke: PH_DIM });  // the chamber
    // a few orbiting motes on the chamber
    const motes = Array.from({ length: 10 }, (_, i) => ({ q: el(svg, "circle",
      { r: 2, fill: PH_BRIGHT, opacity: .7 }), a: i * Math.PI / 5, r: 150 + (i % 3) * 26 }));
    const rings = [], shards = [];
    if (!shattered) {
      el(svg, "ellipse", { cx: CX, cy: CY, rx: 118, ry: 156, fill: FD, stroke: PH, "stroke-width": 2.4 });
      if (cfg.state === "upright")
        el(svg, "line", { x1: CX, y1: CY - 154, x2: CX, y2: CY + 154, stroke: PH_DIM });  // the seam
      if (cfg.state === "reforged")
        for (let k = 0; k < 3; k++) rings.push(el(svg, "ellipse", { cx: CX, cy: CY,
          fill: "none", stroke: PH_BRIGHT, opacity: .5 }));
    } else {
      el(svg, "ellipse", { cx: CX, cy: CY, rx: 118, ry: 156, fill: "none",
        stroke: PH_DIM, "stroke-dasharray": "5 5" });                    // the ghost home
      for (let s = 0; s < 6; s++) {
        const a = s * Math.PI / 3;
        shards.push({ q: el(svg, "polygon", { fill: FD, stroke: PH, "stroke-width": 1.8, points:
          `${CX},${CY} ${CX + Math.cos(a) * 118},${CY + Math.sin(a) * 156} ` +
          `${CX + Math.cos(a + 1.05) * 118},${CY + Math.sin(a + 1.05) * 156}` }), a });
      }
    }
    // the fungus tenant -- a knot with coiling tendrils -- when present
    const tends = [];
    if (tenant) {
      el(svg, "circle", { cx: CX, cy: CY, r: 20, fill: FUNGUS, "fill-opacity": .5, stroke: FUNGUS });
      for (let i = 0; i < 6; i++)
        tends.push({ q: el(svg, "polyline", { fill: "none", stroke: FUNGUS, "stroke-width": 2 }), a: i * Math.PI / 3 });
    }
    let cells = null;
    if (cfg.gauge) {
      label(svg, 40, 556, 19, PH_DIM).textContent = cfg.gauge;
      cells = Array.from({ length: 10 }, (_, i) => el(svg, "rect", { x: 210 + i * 36, y: 540,
        width: 26, height: 20, fill: PH, "fill-opacity": .8 }));
    }
    const calls = (cfg.callouts || []).slice(0, 3).map((c, i) => label(svg, 40, 584 + i * 28, 16, PH_DIM));
    const fw = cfg.footer, cut = fw.length > 34 ? fw.lastIndexOf(" ", 34) : -1;
    const fL1 = cut > 0 ? fw.slice(0, cut) : fw, fL2 = cut > 0 ? fw.slice(cut + 1) : "";
    const foot1 = label(svg, 320, 668, 21, FUNGUS); foot1.setAttribute("text-anchor", "middle");
    const foot2 = label(svg, 320, 698, 21, FUNGUS); foot2.setAttribute("text-anchor", "middle");
    const doWipe = wipe(svg, 640, 720, 2, 12);
    clock(t => {
      const T = t % 220;
      doWipe(T);
      typeOn(hdr, cfg.header, T, 4, 1.2);
      motes.forEach((m, i) => { const a = m.a + t * .01 * (1 + i % 2);
        m.q.setAttribute("cx", CX + Math.cos(a) * m.r); m.q.setAttribute("cy", CY + Math.sin(a) * m.r); });
      rings.forEach((r, k) => { const g = ((t * 2 + k * 30) % 90) / 90;
        r.setAttribute("rx", 118 * g); r.setAttribute("ry", 156 * g); r.setAttribute("opacity", .6 * (1 - g)); });
      shards.forEach((s, i) => {                              // drift out (tenant) or breathe (vacant)
        const out = cfg.state === "shatter-tenant" ? 40 + (t % 120) * .4 : Math.abs(Math.sin(t * .05 + i)) * 22;
        s.q.setAttribute("transform", `translate(${Math.cos(s.a) * out} ${Math.sin(s.a) * out})`);
      });
      tends.forEach((td, i) => {
        const pts = [[CX, CY]];
        for (let q = 1; q <= 4; q++) {
          const sway = Math.sin(t * .1 + q + i) * 5;
          pts.push([CX + Math.cos(td.a) * q * 22 + sway, CY + Math.sin(td.a) * q * 22]);
        }
        td.q.setAttribute("points", pts.map(p => p.join(",")).join(" "));
      });
      if (cells) { const on = 4 + Math.floor(3 * (1 + Math.sin(t * .08))); cells.forEach((r, i) => r.setAttribute("fill-opacity", i < on ? .8 : .1)); }
  ...
  // the spawn of brain, phone layout -- shared by spawn-a/b/c-m.
  // cfg = { key, header, state:'listen'|'step'|'felled', footer }
  function spawnCardM(svg, cfg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 23, PH_BRIGHT);
    const BR = stipple(svg, "dots-" + cfg.key, FUNGUS, 1.2);
    const felled = cfg.state === "felled";
    el(svg, "line", { x1: 0, y1: 520, x2: 640, y2: 520, stroke: PH_DIM });   // ground
    const body = el(svg, "g", {});
    const B = (n, a) => el(body, n, a);
    if (!felled) {
      // the worn canopic jar-head, held level
      B("path", { fill: "none", stroke: PH, "stroke-width": 2.2,
        d: "M 286 250 L 282 180 Q 282 150 320 148 Q 358 150 358 180 L 354 250 Z" });
      B("ellipse", { cx: 320, cy: 250, rx: 40, ry: 10, fill: "none", stroke: PH });
      B("rect", { x: 300, y: 158, width: 40, height: 8, fill: "none", stroke: PH_DIM }); // worn label
      // the risen brain-body
      B("path", { fill: BR, stroke: FUNGUS, "stroke-width": 2, d:
        "M 262 268 Q 250 320 272 372 Q 320 396 368 372 Q 390 320 378 268 Q 320 250 262 268 Z" });
      B("path", { d: "M 285 300 Q 320 288 355 300 M 290 330 Q 320 320 350 330", fill: "none", stroke: FUNGUS, opacity: .7 });
      // two backward-kneed wading-bird legs
      const legs = [-1, 1].map(s => B("polyline", { fill: "none", stroke: PH, "stroke-width": 2.6, "stroke-linecap": "round" }));
      body._legs = legs;
    } else {
      // squashed brain on the ground, legs splayed, jar-head rolled aside
      B("path", { fill: BR, stroke: FUNGUS, "stroke-width": 2, d:
        "M 250 500 Q 240 470 300 466 Q 360 464 380 490 Q 388 508 340 512 Q 270 516 250 500 Z" });
      B("polyline", { fill: "none", stroke: PH, "stroke-width": 2.4, points: "300,486 250,470 214,486" });
      B("polyline", { fill: "none", stroke: PH, "stroke-width": 2.4, points: "352,486 402,468 440,484" });
      B("path", { fill: "none", stroke: PH, "stroke-width": 2.2,
        d: "M 470 512 L 462 470 Q 462 448 500 446 Q 534 448 534 470 L 528 512 Z", transform: "rotate(64 500 480)" });
    }
    // last thoughts / ears -- a couple of small pulses above the head
    const pulses = Array.from({ length: 3 }, (_, i) =>
      el(svg, "circle", { cx: 320 + (i - 1) * 26, cy: 120, r: 5, fill: "none", stroke: PH_BRIGHT }));
    const fw = cfg.footer, cut = fw.length > 34 ? fw.lastIndexOf(" ", 34) : -1;
    const fL1 = cut > 0 ? fw.slice(0, cut) : fw, fL2 = cut > 0 ? fw.slice(cut + 1) : "";
    const foot1 = label(svg, 320, 616, 22, FUNGUS); foot1.setAttribute("text-anchor", "middle");
    const foot2 = label(svg, 320, 648, 22, FUNGUS); foot2.setAttribute("text-anchor", "middle");
    const doWipe = wipe(svg, 640, 720, 2, 12);
    clock(t => {
      const T = t % 200;
      doWipe(T);
      typeOn(hdr, cfg.header, T, 4, 1.2);
      if (!felled && body._legs) {
        const jounce = cfg.state === "step" ? [0, 2, 3, 2][t % 4] : 0;
        body.setAttribute("transform", `translate(0 ${jounce})`);
        body._legs.forEach((lg, i) => {
          const s = i ? 1 : -1;
          const th = cfg.state === "step" ? ((t % 12) / 12 + i * .5) * Math.PI * 2 : 0;
          const fx = 320 + s * 26 + Math.cos(th) * 14;
          lg.setAttribute("points", `${320 + s * 20},372 ${320 + s * 40},430 ${fx},520`);  // backward knee
        });
      }
      pulses.forEach((p, i) => {
        const on = felled ? Math.max(0, 1 - (t % 60) / 60) * .6 : (cfg.state === "listen" ? (t % 30 < 15 ? 1 : .2) : .5);
        p.setAttribute("opacity", on);
        p.setAttribute("r", 4 + (t % 24) * .2 + i);
      });
      typeOn(foot1, fL1, T, 130, 1.6);
      typeOn(foot2, fL2, T, 130 + fL1.length, 1.6);
    });
  ...
  // the spawn of guts, phone layout -- shared by guts-a/b/c/lash-m.
  // cfg = { key, header, state:'sway'|'drip'|'claim'|'lash', callouts, footer }
  function gutsCardM(svg, cfg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 20, PH_BRIGHT);
    const GD = stipple(svg, "dots-" + cfg.key, PH_DIM, 1.1);
    const GUT = "#4ee0a8";
    const close = cfg.state === "drip", claim = cfg.state === "claim", lash = cfg.state === "lash";
    el(svg, "line", { x1: 0, y1: 540, x2: 640, y2: 540, stroke: PH_DIM });
    const S = close ? 1.5 : 1, CX = 320, TOP = close ? 150 : 150;
    const fig = el(svg, "g", { transform: `translate(${CX} ${TOP}) scale(${S})` });
    const F = (n, a) => el(fig, n, a);
    if (!claim) {
      // the falcon-jar helm
      F("path", { fill: GD, stroke: PH, "stroke-width": 2, d: "M -46 90 L -42 20 Q -42 -8 0 -10 Q 42 -8 42 20 L 46 90 Z" });
      F("polyline", { fill: "none", stroke: PH, "stroke-width": 2, points: "-8,-6 20,-18 6,2" }); // beak
      // the eye-cluster face
      const eyes = [];
      for (let i = 0; i < 6; i++) {
        const ex = -30 + (i % 3) * 30, ey = 108 + Math.floor(i / 3) * 26;
        eyes.push(F("circle", { cx: ex, cy: ey, r: 7, fill: BG, stroke: PH, "stroke-width": 1.6 }));
        F("circle", { cx: ex, cy: ey, r: 2.6, fill: PH_BRIGHT });
      }
      fig._eyes = eyes;
      // the green gut torus collar
      F("ellipse", { cx: 0, cy: 168, rx: 58, ry: 20, fill: "none", stroke: GUT, "stroke-width": 3 });
      F("ellipse", { cx: 0, cy: 168, rx: 34, ry: 11, fill: "none", stroke: GUT, "stroke-width": 1.6, opacity: .6 });
      // the tendril skirt
      const skirt = [];
      for (let i = 0; i < 7; i++) skirt.push(F("polyline", { fill: "none", stroke: GUT, "stroke-width": 2.2 }));
      fig._skirt = skirt;
      // the intestine arm (right)
      fig._arm = F("polyline", { fill: "none", stroke: GUT, "stroke-width": 3, "stroke-linecap": "round" });
    } else {
      // collapsed; the falcon-jar helm toppled, settling upright below
      F("path", { fill: GD, stroke: PH, "stroke-width": 2, d: "M -40 130 L -36 66 Q -36 40 0 38 Q 36 40 36 66 L 40 130 Z" });
      F("ellipse", { cx: 0, cy: 130, rx: 40, ry: 10, fill: "none", stroke: PH });
      F("polyline", { fill: "none", stroke: PH, "stroke-width": 2, points: "-6,44 20,32 4,52" });
    }
    // drips + the red lash burst
    const drips = Array.from({ length: 4 }, (_, i) =>
      el(svg, "circle", { r: 3, fill: GUT, cx: 280 + i * 30, cy: 400, opacity: 0 }));
    const burst = lash ? el(svg, "g", { opacity: 0 }) : null;
    if (burst) for (let i = 0; i < 8; i++) { const a = i * Math.PI / 4;
      el(burst, "line", { x1: 540, y1: 300, x2: 540 + Math.cos(a) * 26, y2: 300 + Math.sin(a) * 26, stroke: "#ff5a5a", "stroke-width": 2.4 }); }
    const calls = (cfg.callouts || []).slice(0, 4).map((c, i) => label(svg, 40, 576 + i * 28, 16, PH_DIM));
    const fw = cfg.footer, cut = fw.length > 34 ? fw.lastIndexOf(" ", 34) : -1;
    const fL1 = cut > 0 ? fw.slice(0, cut) : fw, fL2 = cut > 0 ? fw.slice(cut + 1) : "";
    const yF = (cfg.callouts && cfg.callouts.length) ? 700 : 636;
    const foot1 = label(svg, 320, yF - 26, 21, FUNGUS); foot1.setAttribute("text-anchor", "middle");
    const foot2 = label(svg, 320, yF + 2, 21, FUNGUS); foot2.setAttribute("text-anchor", "middle");
    const doWipe = wipe(svg, 640, 720, 2, 12);
    clock(t => {
      const T = t % 200;
      doWipe(T);
      typeOn(hdr, cfg.header, T, 4, 1.1);
      if (!claim) {
        const sway = lash ? 0 : Math.sin(t * .08) * (close ? 4 : 10);
        fig.setAttribute("transform", `translate(${CX + sway} ${TOP}) scale(${S})`);
        if (fig._eyes) fig._eyes.forEach((e, i) => e.setAttribute("opacity", (t + i * 3) % 24 < 3 ? .2 : 1));
        if (fig._skirt) fig._skirt.forEach((sk, i) => {
          const bx = -54 + i * 18, pts = [[bx, 186]];
          for (let q = 1; q <= 3; q++) pts.push([bx + Math.sin(t * .12 + q + i) * 5, 186 + q * 24]);
          sk.setAttribute("points", pts.map(p => p.join(",")).join(" "));
        });
        if (fig._arm) {
          const reach = lash ? Math.min(1, (t % 60) / 12) : 0;
          const ex = 60 + reach * 150, ey = 150 - reach * 30;
          fig._arm.setAttribute("points", `40,150 ${52 + reach * 20},170 ${ex},${ey}`);
        }
      }
      drips.forEach((d, i) => { const k = (t * 2 + i * 20) % 100;
        d.setAttribute("cy", 380 + k * 1.4); d.setAttribute("opacity", k > 84 ? 0 : .7); });
      if (burst) burst.setAttribute("opacity", (t % 60) > 12 && (t % 60) < 22 ? 1 : 0);
  ...
  // the ulfire lantern x-raying a slab, phone layout -- shared by ulfire/-u-m.
  // cfg = { key, header, lit, callouts, footer }
  function ulfireCardM(svg, cfg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 24, PH_BRIGHT);
    const SD = stipple(svg, "dots-" + cfg.key, PH, 1);
    el(svg, "rect", { x: 150, y: 130, width: 340, height: 380, fill: SD, stroke: PH, "stroke-width": 2.4 });
    // the interior anatomy (only meaningful when the beam is lit)
    let coffin = null, mystic = null;
    if (cfg.lit) {
      const gi = el(svg, "g", { opacity: .22 });
      el(gi, "polyline", { fill: "none", stroke: FUNGUS, "stroke-width": 2,
        points: "320,150 300,220 336,300 306,380 326,470" });      // the fungal channel
      coffin = el(gi, "ellipse", { cx: 320, cy: 452, rx: 40, ry: 30, fill: "none", stroke: PH, "stroke-width": 2 });
      mystic = el(gi, "path", { fill: "none", stroke: PH, "stroke-width": 2, d: "M 286 232 Q 320 176 354 232 Z" });
      el(gi, "circle", { cx: 320, cy: 214, r: 9, fill: PH_BRIGHT });
    }
    // the held lantern, low-left
    const lant = el(svg, "g", { transform: "translate(96 452)" });
    el(lant, "path", { fill: "none", stroke: PH, "stroke-width": 2, d: "M -22 60 L -16 6 L 16 6 L 22 60 Z" });
    el(lant, "rect", { x: -18, y: 0, width: 36, height: 8, fill: "none", stroke: PH });
    el(lant, "line", { x1: 0, y1: -14, x2: 0, y2: 0, stroke: PH });
    const flame = cfg.lit ? el(lant, "polygon", { points: "-6,28 0,10 6,28", fill: "#dff2ff" })
      : el(lant, "line", { x1: 0, y1: 30, x2: 0, y2: 22, stroke: PH_DIM, "stroke-width": 2 });
    let cone = null, band = null;
    const HUES = ["#ff5a5a", "#ff9a3c", "#ffd76a", "#4ee0a8", "#4db8ff", "#a06aff"];
    if (cfg.lit) {
      cone = el(svg, "polygon", { fill: "none", "stroke-width": 1.6, opacity: .5,
        points: "104,460 150,150 490,150 490,510 150,510" });
      band = el(svg, "rect", { y: 130, height: 380, width: 64, fill: PH_BRIGHT, opacity: .1 });
    }
    const calls = (cfg.callouts || []).slice(0, 3).map((c, i) => label(svg, 40, 590 + i * 30, 16, PH_DIM));
    const fw = cfg.footer, cut = fw.length > 34 ? fw.lastIndexOf(" ", 34) : -1;
    const fL1 = cut > 0 ? fw.slice(0, cut) : fw, fL2 = cut > 0 ? fw.slice(cut + 1) : "";
    const foot1 = label(svg, 320, 690, 21, FUNGUS); foot1.setAttribute("text-anchor", "middle");
    const foot2 = label(svg, 320, 690, 21, FUNGUS); foot2.setAttribute("text-anchor", "middle");
    if (fL2) { foot1.setAttribute("y", 668); }
    const doWipe = wipe(svg, 640, 720, 2, 12);
    clock(t => {
      const T = t % 220;
      doWipe(T);
      typeOn(hdr, cfg.header, T, 4, 1.2);
      if (cfg.lit) {
        const hue = HUES[Math.floor(t / 6) % HUES.length];
        cone.setAttribute("stroke", hue);
        flame.setAttribute("fill", hue);
        const bx = 150 + ((t * 5) % 340) - 32;
        band.setAttribute("x", bx); band.setAttribute("fill", hue);
        const near = 1 - Math.min(1, Math.abs(bx + 32 - 320) / 130);
        if (coffin) coffin.setAttribute("stroke", near > .5 ? PH_BRIGHT : PH);
        if (mystic) mystic.setAttribute("stroke", near > .5 ? PH_BRIGHT : PH);
      }
      (cfg.callouts || []).slice(0, 3).forEach((c, i) => typeOn(calls[i], c, T, 40 + i * 16, 1.6));
      typeOn(foot1, fL1, T, 150, 1.7);
      typeOn(foot2, fL2, T, 150 + fL1.length, 1.7);
  ...
  /* ---------------- 04-m: PTHALO-JACKAL (phone twin) ----------------
     The creature entry, relaid for phone: the jackal drawn big up top, then the
     guide callouts and the verdict set large below. */
  FIG._define("jackal-m", "svg", function (svg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 28, PH_BRIGHT);
    const kick = label(svg, 620, 34, 15, PH_DIM); kick.setAttribute("text-anchor", "end");
    kick.textContent = "FAUNA / DESERT";
    const JDOTS = stipple(svg, "dots-jackalm", PH, 1.1);
    // the jackal, prowling left, drawn big -- a group that bobs on the gait
    const gJ = el(svg, "g", {});
    const J = (name, attrs) => el(gJ, name, attrs);
    // top contour: nose -> skull -> swept ears -> back -> hip -> tail base
    J("polyline", { fill: "none", stroke: PH, "stroke-width": 2.4, points:
      "70,300 96,282 128,268 150,256 158,250 148,168 190,224 " +
      "202,228 246,164 274,220 320,232 402,224 470,236 512,262 520,280" });
    // underside: jaw -> throat -> chest -> belly -> flank
    J("polyline", { fill: "none", stroke: PH, "stroke-width": 2.4, points:
      "70,300 78,326 110,338 156,334 214,310 280,296 330,320 360,360 " +
      "440,372 500,368 520,340" });
    // the bared grin
    J("polyline", { fill: "none", stroke: PH, "stroke-width": 1.8, points:
      "82,312 92,326 102,310 112,326 122,312 132,325 142,314 152,326" });
    // the eye: yellow, slanted, doing sums
    J("polygon", { points: "150,272 168,262 186,272 168,280", fill: "#ffd76a",
      stroke: "#ffd76a", "stroke-width": 1 });
    const eye = J("circle", { cx: 170, cy: 272, r: 3, fill: BG });
    // fur along the spine
    for (let i = 0; i < 14; i++) {
      const x = 250 + i * 20, len = i < 5 ? 18 - i * 2 : 8;
      J("line", { x1: x, y1: 224 + i * 1.2, x2: x - 7, y2: 224 + i * 1.2 - len,
        stroke: PH, "stroke-width": 1.4 });
    }
    // the brush tail, halftoned, trailing
    J("polygon", { fill: JDOTS, stroke: PH, "stroke-width": 1.8, points:
      "520,280 566,320 604,372 566,382 512,340 486,300" });
    // the walking legs
    // the legs -- long and thin, a proper canine gait (haunch/shoulder down to
    // the ground band), near pair brighter than far
    const LEGS = [[206, 286], [258, 282], [416, 290], [460, 286]].map(([ax, ay], i) => ({
      ax, ay, ph: i * .5, near: i % 2,
      leg: J("polyline", { fill: "none", stroke: PH, "stroke-width": i % 2 ? 2.8 : 2,
        "stroke-linecap": "round", opacity: i % 2 ? 1 : .5 }),
    }));
    // the ground ticks, sliding under the walk
    const gGround = el(svg, "g", {});
    for (let x = 30; x < 610; x += 20)
      el(gGround, "line", { x1: x, y1: 454, x2: x + 9, y2: 454, stroke: PH_DIM });
    // the guide callouts, big
    const CALL = ["EYES: DOING SUMS", "NOSE: FINDS MEAT AT TWO ROOMS", "COAT: PTHALO No.9"];
    const calls = CALL.map((c, i) => label(svg, 40, 502 + i * 38, 21, PH));
    // the PATIENCE gauge + verdict
    label(svg, 40, 638, 20, PH_DIM).textContent = "PATIENCE";
    const cells = Array.from({ length: 10 }, (_, i) =>
      el(svg, "rect", { x: 200 + i * 34, y: 622, width: 26, height: 20,
        fill: PH, "fill-opacity": .8 }));
    const verdict = label(svg, 320, 692, 22, FUNGUS); verdict.setAttribute("text-anchor", "middle");
    const doWipe = wipe(svg, 640, 720, 2, 10);
    clock(t => {
      const T = t % 220;
      doWipe(T);
      typeOn(hdr, "PTHALO-JACKAL", T, 4, 1.3);
      const bob = [0, 1, 2, 1, 0, -1, -2, -1][t % 8] * 1.1;
      gJ.setAttribute("transform", `translate(0 ${bob})`);
      eye.setAttribute("opacity", t % 40 < 2 ? 0 : 1);         // a rare blink
      LEGS.forEach(L => {
        const th = ((t % 16) / 16 + L.ph) * Math.PI * 2;
        const fx = L.ax - 4 + 16 * Math.cos(th);
        const fy = 454 - Math.max(0, Math.sin(th)) * 20;
        // knee ~55% down the leg, kicked forward, so the shank reads long
        const kx = L.ax + (fx - L.ax) * .5 + (L.knee || 0);
        const ky = L.ay + bob + (fy - L.ay - bob) * .52;
        L.leg.setAttribute("points", `${L.ax},${L.ay + bob} ${kx},${ky} ${fx},${fy}`);
      });
      gGround.setAttribute("transform", `translate(${(t * 5) % 20} 0)`);
      calls.forEach((n, i) => typeOn(n, CALL[i], T, 30 + i * 16, 1.7));
      const left = Math.max(0, 10 - Math.floor(Math.max(0, T - 40) / 12));
      cells.forEach((r, i) => r.setAttribute("fill-opacity", i < left ? .8 : .08));
      verdict.textContent = left === 0
        ? "TRIBUTE ACCEPTED: PEACE, PURCHASED DEEP"
        : left <= 3 ? "ADVICE: PRODUCE THE DATES" : "";
  ...
  /* ---------------- 03-C-m: the canopic system, supine (phone twin) ----------------
     The five-jar procedure relaid as a vertical inventory, so each organ->jar
     pairing reads big on a phone. Reuses the headGlyph stopper heads. */
  FIG._define("canopic-c-m", "svg", function (svg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 27, PH_BRIGHT);
    const kick = label(svg, 620, 34, 15, PH_DIM); kick.setAttribute("text-anchor", "end");
    kick.textContent = "RITES / SUPINE";
    const JARS = [
      ["LUNGS", "BABOON"], ["LIVER", "HUMAN"], ["EYES", "MANTIS"],
      ["INTESTINES", "FALCON"], ["BRAIN", "JACKAL"],
    ];
    const rows = JARS.map(([organ, head], i) => {
      const y = 130 + i * 100;                                 // 130..530
      // the jar body, and the animal-head stopper (glyph drawn ~y273, moved up)
      el(svg, "polygon", { points: `60,${y + 58} 120,${y + 58} 110,${y + 14} 70,${y + 14}`,
        fill: PH, "fill-opacity": .10, stroke: PH, "stroke-width": 1.8 });
      const g = el(svg, "g", { transform: `translate(0 ${y - 273})` });
      headGlyph(g, head, 90);
      const organL = label(svg, 158, y - 2, 26, PH_BRIGHT);
      const headL = label(svg, 158, y + 26, 17, PH_DIM);
      headL.textContent = "— " + head + " STOPPER";
      return { organ, organL, i };
    });
    ...
    clock(t => {
      const T = t % 220;
      doWipe(T);
      typeOn(hdr, "THE CANOPIC SYSTEM", T, 4, 1.3);
      rows.forEach(r => typeOn(r.organL, r.organ, T, 20 + r.i * 16, 1.6));
      typeOn(foot1, "PLEASE REMOVE ORGANS", T, 130, 1.6);
      typeOn(foot2, "BEFORE INTERMENT.", T, 150, 1.6);
    });
  });
  ...
  /* ---------------- 17-C-m: the approach, thin curls (phone twin) ----------------
     A near-spectacle card (its only prose is the footer), so the twin draws the
     crowned idol big on a taller portrait and sets header + footer large. */
  FIG._define("ext1c-m", "svg", function (svg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 30, PH_BRIGHT);
    const kick = label(svg, 620, 34, 15, PH_DIM); kick.setAttribute("text-anchor", "end");
    kick.textContent = "AZURE STONE";
    const XDOTS = stipple(svg, "dots-ext1cm", PH, .9);
    // the fungus horizon, and the hatched underworld it stands over
    el(svg, "line", { x1: 0, y1: 500, x2: 640, y2: 500, stroke: FUNGUS,
      "stroke-width": 2, opacity: .8 });
    el(svg, "rect", { x: 0, y: 502, width: 640, height: 130, fill: XDOTS, opacity: .3 });
    // THE IDOL: a tall crowned monolith, a face on its body, mouth a door down
    el(svg, "polygon", { fill: XDOTS, stroke: PH, "stroke-width": 2.4, points:
      "250,168 262,120 286,142 300,110 320,132 340,110 354,142 378,120 390,168 " +
      "404,320 396,430 402,500 238,500 244,430 236,320" });
    el(svg, "circle", { cx: 320, cy: 138, r: 3, fill: PH_BRIGHT });     // the crown glint
    el(svg, "circle", { cx: 292, cy: 320, r: 16, fill: "none", stroke: PH, "stroke-width": 1.8 });
    el(svg, "circle", { cx: 348, cy: 320, r: 16, fill: "none", stroke: PH, "stroke-width": 1.8 });
    el(svg, "circle", { cx: 292, cy: 320, r: 5, fill: PH_BRIGHT });
    el(svg, "circle", { cx: 348, cy: 320, r: 5, fill: PH_BRIGHT });
    el(svg, "path", { d: "M 288 500 L 288 424 Q 320 392 352 424 L 352 500 Z",
      fill: BG, stroke: PH, "stroke-width": 2 });                       // the door-mouth
    el(svg, "line", { x1: 288, y1: 500, x2: 352, y2: 500, stroke: PH_BRIGHT, "stroke-width": 2 });
    // the thin curls, rising from the crown, orange -- weeping their drips
    const TEND = [[300, 120, -1.4], [312, 112, -.5], [328, 112, .5], [340, 120, 1.4]];
    const tendrils = TEND.map(() => el(svg, "polyline", { fill: "none",
      stroke: FUNGUS, "stroke-width": 2.4 }));
    const drips = TEND.map(() => el(svg, "circle", { r: 2.4, fill: FUNGUS }));
    // a small walker, coming in along the horizon to stand before it
    const walk = el(svg, "g", {});
    el(walk, "circle", { cx: 0, cy: -22, r: 5, fill: PH_BRIGHT });
    el(walk, "line", { x1: 0, y1: -17, x2: 0, y2: 2, stroke: PH_BRIGHT, "stroke-width": 2 });
    const walkLegs = el(walk, "polyline", { fill: "none", stroke: PH_BRIGHT, "stroke-width": 1.8 });
    // the footer, big and wrapped
    const F1 = label(svg, 320, 596, 24, FUNGUS); F1.setAttribute("text-anchor", "middle");
    const F2 = label(svg, 320, 632, 24, FUNGUS); F2.setAttribute("text-anchor", "middle");
    const doWipe = wipe(svg, 640, 720, 2, 10);
    clock(t => {
      const T = Math.min(t, 259);
      doWipe(T);
      typeOn(hdr, "THE APPROACH", T, 4, 1.3);
      tendrils.forEach((n, i) => {
        const [x0, y0, ph] = TEND[i];
        const pts = [[x0, y0]];
        for (let q = 1; q <= 5; q++) {
          const sway = Math.sin(t * .16 + q * .9 + ph * 2) * 4 * q * .35;
          pts.push([x0 + ph * q * 8 + sway, y0 - q * 11]);
        }
        n.setAttribute("points", pts.map(p => p.join(",")).join(" "));
        const d = drips[i], dk = (t * 2 + i * 17) % 60;
        d.setAttribute("cx", x0 + ph * 22); d.setAttribute("cy", y0 - 55 + dk);
        d.setAttribute("opacity", dk > 46 ? 0 : .9);
      });
      const wx = Math.min(150, 24 + T * 1.1);
      walk.setAttribute("transform", `translate(${wx} 500)`);
      const st = (t % 4 < 2) ? 6 : -6;
      walkLegs.setAttribute("points", wx < 150 ? `${-st},16 0,2 ${st},16` : "-4,16 0,2 4,16");
      typeOn(F1, "HIS SENTENCE CONTINUES", T, 150, 1.6);
      typeOn(F2, "UNDERGROUND.", T, 172, 1.6);
    });
  });
  ...
  /* ---------------- 17-D-m: the approach, elevations (phone twin) ----------------
     The survey cycles three faces of the slab; the twin draws each big, one at a
     time, with a large two-line caption -- the elevation prose the desktop card
     types small. */
  FIG._define("ext1e-m", "svg", function (svg) {
    ...
    const hdr = label(svg, 20, 34, 30, PH_BRIGHT);
    const kick = label(svg, 620, 34, 15, PH_DIM); kick.setAttribute("text-anchor", "end");
    kick.textContent = "SURVEY / BOTH DOORS";
    const EDOTS = stipple(svg, "dots-ext1em", PH, .9);
    el(svg, "line", { x1: 30, y1: 520, x2: 610, y2: 520, stroke: PH, "stroke-width": 1.6 });
    // the slab, face-on (hidden for the plan view)
    const slab = el(svg, "polygon", { fill: EDOTS, stroke: PH, "stroke-width": 2,
      points: "196,172 444,172 456,520 184,520" });
    // the crown, weeping tendrils -- on the elevations only
    const crownG = el(svg, "g", {});
    el(crownG, "polyline", { fill: "none", stroke: PH, "stroke-width": 2, points:
      "268,172 282,140 300,130 312,134 320,118 332,116 340,130 352,134 366,140 372,172" });
    el(crownG, "circle", { cx: 322, cy: 126, r: 2.4, fill: PH_BRIGHT });
    const TEND = [[312, 124, -1.6], [320, 120, -.5], [330, 122, .6], [338, 126, 1.6]];
    const tendrils = TEND.map(() => el(crownG, "polyline", { fill: "none",
      stroke: FUNGUS, "stroke-width": 2.4 }));
    // the door at the base, shared by both face elevations
    const door = parent => {
      el(parent, "path", { d: "M 288 520 L 288 452 Q 320 424 352 452 L 352 520 Z",
        fill: BG, stroke: PH, "stroke-width": 2 });
      el(parent, "line", { x1: 288, y1: 520, x2: 352, y2: 520,
        stroke: PH_BRIGHT, "stroke-width": 2 });
    };
    // WEST: the Autarch, young -- a smooth swaddled oval, eyes closed
    const west = el(svg, "g", {});
    el(west, "ellipse", { cx: 320, cy: 312, rx: 78, ry: 94, fill: "none",
      stroke: PH, "stroke-width": 2 });
    el(west, "path", { d: "M 276 294 Q 292 282 308 294", fill: "none", stroke: PH, "stroke-width": 2 });
    el(west, "path", { d: "M 332 294 Q 348 282 364 294", fill: "none", stroke: PH, "stroke-width": 2 });
    el(west, "path", { d: "M 308 320 Q 320 330 332 320", fill: "none", stroke: PH_DIM });
    [0, 1, 2].forEach(k => el(west, "path", { fill: "none", stroke: PH_DIM,
      d: `M ${252 - k * 6} ${256 + k * 34} Q 320 ${238 + k * 34} ${388 + k * 6} ${256 + k * 34}` }));
    door(west);
    // EAST: the helmed warrior -- a great helm, eye slits, riveted band
    const east = el(svg, "g", {});
    el(east, "path", { fill: "none", stroke: PH, "stroke-width": 2.2, d:
      "M 262 440 L 256 262 Q 256 184 320 178 Q 384 184 384 262 L 378 440 " +
      "Q 352 452 320 454 Q 288 452 262 440 Z" });
    el(east, "line", { x1: 320, y1: 178, x2: 320, y2: 308, stroke: PH, "stroke-width": 2.2 });
    el(east, "rect", { x: 262, y: 284, width: 44, height: 14, fill: BG, stroke: PH, "stroke-width": 1.8 });
    el(east, "rect", { x: 334, y: 284, width: 44, height: 14, fill: BG, stroke: PH, "stroke-width": 1.8 });
    el(east, "line", { x1: 264, y1: 364, x2: 376, y2: 364, stroke: PH, "stroke-width": 1.6 });
    [284, 312, 340, 368].forEach(x => el(east, "circle", { cx: x, cy: 374, r: 2.2, fill: PH_BRIGHT }));
    door(east);
    // SUMMIT: the plan view -- the upturned face, the mouth a chimney
    const plan = el(svg, "g", {});
    el(plan, "rect", { x: 196, y: 186, width: 248, height: 320, rx: 20, fill: "none",
      stroke: PH, "stroke-width": 2 });
    el(plan, "ellipse", { cx: 320, cy: 336, rx: 84, ry: 104, fill: "none", stroke: PH, "stroke-width": 2 });
    el(plan, "circle", { cx: 288, cy: 308, r: 11, fill: "none", stroke: PH, "stroke-width": 2 });
    el(plan, "circle", { cx: 352, cy: 308, r: 11, fill: "none", stroke: PH, "stroke-width": 2 });
    el(plan, "circle", { cx: 288, cy: 308, r: 3.6, fill: PH_BRIGHT });
    el(plan, "circle", { cx: 352, cy: 308, r: 3.6, fill: PH_BRIGHT });
    el(plan, "circle", { cx: 320, cy: 388, r: 22, fill: BG, stroke: PH, "stroke-width": 2 });
    el(plan, "circle", { cx: 320, cy: 388, r: 11, fill: "none", stroke: FUNGUS, "stroke-width": 2 });
    const ptend = Array.from({ length: 8 }, () => el(plan, "polyline", { fill: "none",
      stroke: FUNGUS, "stroke-width": 2 }));
    // the big compass, the two-line caption, and the footer
    const compass = label(svg, 44, 312, 44, PH_DIM);
    const C1 = label(svg, 320, 578, 22, PH); C1.setAttribute("text-anchor", "middle");
    const C2 = label(svg, 320, 608, 22, PH); C2.setAttribute("text-anchor", "middle");
    const foot = label(svg, 320, 666, 23, FUNGUS); foot.setAttribute("text-anchor", "middle");
    const CAPS = [
      ["WEST -- THE AUTARCH, YOUNG.", "HIS MOUTH IS A DOOR."],
      ["EAST -- THE HELMED WARRIOR.", "HIS MOUTH IS A DOOR."],
      ["SUMMIT -- THE OLD MAN, SKYWARD.", "HIS MOUTH IS A CHIMNEY."],
    ];
    const doWipe = wipe(svg, 640, 720, 2, 10);
    clock(t => {
      const T = t % 300;
      doWipe(Math.min(t, 259));
      typeOn(hdr, "THE ELEVATIONS", Math.min(t, 259), 4, 1.3);
      const phase = Math.floor(T / 100) % 3;
      west.setAttribute("opacity", phase === 0 ? 1 : 0);
      east.setAttribute("opacity", phase === 1 ? 1 : 0);
      plan.setAttribute("opacity", phase === 2 ? 1 : 0);
      crownG.setAttribute("opacity", phase === 2 ? 0 : 1);
      slab.setAttribute("opacity", phase === 2 ? 0 : 1);
      compass.textContent = ["W", "E", "UP"][phase];
      const cap = CAPS[phase];
      typeOn(C1, cap[0], T % 100, 8, 2.0);
      typeOn(C2, cap[1], T % 100, 26, 2.0);
      tendrils.forEach((n, i) => {
        const [x0, y0, ph] = TEND[i];
        const pts = [[x0, y0]];
        for (let q = 1; q <= 5; q++) {
          const sway = Math.sin(t * .2 + q * .9 + ph * 2) * 3 * q * .4;
          pts.push([x0 + ph * q * 8 + sway, y0 - q * 11]);
        }
        n.setAttribute("points", pts.map(p => p.join(",")).join(" "));
      });
      ptend.forEach((n, i) => {
        const a = i * Math.PI / 4 + .35;
        const pts = [[320 + Math.cos(a) * 22, 388 + Math.sin(a) * 22]];
        for (let q = 1; q <= 4; q++) {
          const wob = Math.sin(t * .2 + q * 1.1 + i * 1.7) * 5;
          const r = 22 + q * 20;
          pts.push([320 + Math.cos(a) * r, 388 + Math.sin(a) * r + wob * .4]);
        }
        n.setAttribute("points", pts.map(p => p.join(",")).join(" "));
      });
      typeOn(foot, "THREE FACES, TWO DOORS, ONE CHIMNEY.", Math.min(t, 259), 120, 1.6);
    });
  });

  /* ---------------- bats-c-m (phone twin) ---------------- */
  FIG._define("bats-c-m", "svg", function (svg) {
    const ctx = window.TombFigures ? window.TombFigures.context : null;
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 720, fill: BG });
    el(svg, "line", { x1: 20, y1: 44, x2: 620, y2: 44, stroke: PH_DIM });
    const hdr = label(svg, 20, 34, 30, PH_BRIGHT);
    const kick = label(svg, 620, 34, 15, PH_DIM);
    kick.setAttribute("text-anchor", "end");

    // the SUBJECT: one bat hanging, folded among the eleven hundred.
    // Re-composed from the original at ~1.7x so it fills the phone column.
    const subj = el(svg, "g", { transform: "translate(-215 22) scale(1.72)" });
    const RED = "#ff5252";

    // the rock lip and the gripping feet
    el(subj, "polyline", { fill: "none", stroke: PH, "stroke-width": 1.6,
      points: "220,64 300,58 340,60 420,66" });
    el(subj, "polyline", { fill: "none", stroke: PH, "stroke-width": 1.2,
      points: "308,60 306,74 312,70 316,74 314,60" });

    // the neighbors: more of the thousand, folded and dimmer
    [[244, .62, .55], [398, .7, .6], [438, .5, .4]].forEach(([cx, sc, op]) => {
      const g = el(subj, "g", { opacity: op });
      el(g, "polyline", { fill: "none", stroke: PH_DIM, "stroke-width": 1.1,
        points: `${cx - 2},62 ${cx - 4},70 ${cx + 4},70 ${cx + 2},62` });
      el(g, "path", { fill: "none", stroke: PH, "stroke-width": 1.2,
        d: `M ${cx - 2},70 Q ${cx - 18 * sc},${70 + 42 * sc} ` +
          `${cx - 10 * sc},${70 + 96 * sc} Q ${cx - 4 * sc},${70 + 112 * sc} ${cx},${70 + 116 * sc} L ${cx - 2},70` });
      el(g, "path", { fill: "none", stroke: PH, "stroke-width": 1.2,
        d: `M ${cx + 2},70 Q ${cx + 18 * sc},${70 + 42 * sc} ` +
          `${cx + 10 * sc},${70 + 96 * sc} Q ${cx + 4 * sc},${70 + 112 * sc} ${cx},${70 + 116 * sc} L ${cx + 2},70` });
      el(g, "circle", { cx, cy: 70 + 124 * sc, r: 9 * sc, fill: "none",
        stroke: PH, "stroke-width": 1.1 });
    });

    // the resident, in full: folded wings, finger bones, head hung down
    const body = el(subj, "g", {});
    el(body, "path", { fill: "none", stroke: PH, "stroke-width": 1.6,
      d: "M 310 74 Q 282 120 292 196 Q 300 224 310 226 L 310 74" });
    el(body, "path", { fill: "none", stroke: PH, "stroke-width": 1.6,
      d: "M 314 74 Q 342 120 332 196 Q 324 224 314 226 L 314 74" });
    [[296, 100, 294, 180], [301, 92, 299, 196], [323, 92, 325, 196], [328, 100, 330, 180]]
      .forEach(([x1, y1, x2, y2]) =>
        el(body, "line", { x1, y1, x2, y2, stroke: PH_DIM }));
    el(body, "circle", { cx: 312, cy: 238, r: 14, fill: "none", stroke: PH, "stroke-width": 1.6 });
    const earA = el(body, "polygon", { fill: "none", stroke: PH, "stroke-width": 1.4 });
    const earB = el(body, "polygon", { fill: "none", stroke: PH, "stroke-width": 1.4 });
    el(body, "polyline", { fill: "none", stroke: PH_DIM, points: "306,244 312,248 318,244" });

    // every resident gets two red eyes; they open on the last beats
    const pair = (ax, ay, bx, by, r) => {
      const g = el(subj, "g", { opacity: 0 });
    ...
    // the LIGHT gauge, filling toward the waking
    label(svg, 40, 486, 20, PH_DIM).textContent = "LIGHT";
    const cells = Array.from({ length: 10 }, (_, i) =>
      el(svg, "rect", { x: 128 + i * 32, y: 470, width: 24, height: 20,
        fill: "#ffd76a", "fill-opacity": .08 }));

    // the footer caption, in orange, appears when the eyes open
    const foot = label(svg, 40, 668, 24, FUNGUS);

    const doWipe = wipe(svg, 640, 720, 2, 12);
    clock(t => {
      const T = t % 160;
      doWipe(T);
      typeOn(hdr, "THE RESIDENTS", T, 4, 1.3);
      typeOn(kick, "HALL OF YOUTH", T, 6, 1.2);

      const br = [0, 1, 2, 2, 1, 0][Math.floor(t / 4) % 6];
      body.setAttribute("transform", `translate(0 ${br * .8})`);
  ...
  /* ---------------- cylinders-b-m (phone twin) ---------------- */
  FIG._define("cylinders-b-m", "svg", function (svg) {
    cylsCardM(svg, { key: "cylinders-b-m", header: "THE BURIAL CYLINDERS (INTACT)", kicker: "GUARD ISSUE / INTACT", burst: [], footer: "AS THE TOMBWRIGHTS LEFT THEM. NOTHING HAS COME SCAVENGING. YET." });
  });

  /* ---------------- cylinders-m (phone twin) ---------------- */
  FIG._define("cylinders-m", "svg", function (svg) {
    cylsCardM(svg, { key: "cylinders-m", header: "THE BURIAL CYLINDERS", kicker: "GUARD ISSUE / TWO BURST", burst: ["CERULEAN", "AMBER"], footer: "KIT OUTLASTS ITS OWNERS." });
  });

  /* ---------------- cyl-c-m (phone twin) ---------------- */
  FIG._define("cyl-c-m", "svg", function (svg) {
    cylsCardM(svg, { key: "cyl-c-m", header: "THE BURIAL CYLINDERS, CERULEAN DOWN", kicker: "GUARD ISSUE / ONE BURST", burst: ["CERULEAN"], footer: "THE BLADE FIRST. AMBER NEXT, IF YOU LIKE BREATHING." });
  });

  /* ---------------- cyl-a-m (phone twin) ---------------- */
  FIG._define("cyl-a-m", "svg", function (svg) {
    cylsCardM(svg, { key: "cyl-a-m", header: "THE BURIAL CYLINDERS, AMBER DOWN", kicker: "GUARD ISSUE / ONE BURST", burst: ["AMBER"], footer: "AMBER FIRST. EXACTLY AS THE MANUAL WOULD HAVE IT." });
  });

  /* ---------------- cyl-v-m (phone twin) ---------------- */
  FIG._define("cyl-v-m", "svg", function (svg) {
    cylsCardM(svg, { key: "cyl-v-m", header: "THE BURIAL CYLINDERS, VIRIDIAN DOWN", kicker: "GUARD ISSUE / ONE BURST", burst: ["VIRIDIAN"], footer: "BOOTS FIRST: A SCAVENGER WHO PLANS AHEAD." });
  });

  /* ---------------- cyl-o-m (phone twin) ---------------- */
  FIG._define("cyl-o-m", "svg", function (svg) {
    cylsCardM(svg, { key: "cyl-o-m", header: "THE BURIAL CYLINDERS, ORANGE DOWN", kicker: "GUARD ISSUE / ONE BURST", burst: ["ORANGE"], footer: "ORANGE FIRST. THE BLOOM THANKS YOU FOR YOUR LUNGS." });
  });

  /* ---------------- jar-mantis-m (phone twin) ---------------- */
  FIG._define("jar-mantis-m", "svg", function (svg) {
    jarCardM(svg, { key: "jar-mantis-m", head: "MANTIS", header: "THE MANTIS JAR (SPLIT, LISTENING)", callouts: ["EYES: COMPOUND. THE FACETS STILL COUNT", "THE SPLIT: SOMETHING BUDS IN IT", "IT AMPLIFIES. MAKE NO SOUND HERE"], footer: "IT LISTENS. THEN IT SINGS." });
  });

  /* ---------------- jar-jackal-m (phone twin) ---------------- */
  FIG._define("jar-jackal-m", "svg", function (svg) {
    jarCardM(svg, { key: "jar-jackal-m", head: "JACKAL", header: "THE JACKAL JAR (THE BRAIN, STRANGELY)", callouts: ["EARS: STILL LISTENING", "CONTENTS: HIS THOUGHTS, JARRED", "TASTE: SOMEBODY'S BIOGRAPHY"], footer: "APPETITES AND THOUGHTS, JARRED SEPARATELY. BOTH GOT UP." });
  });

  /* ---------------- jar-falcon-m (phone twin) ---------------- */
  FIG._define("jar-falcon-m", "svg", function (svg) {
    jarCardM(svg, { key: "jar-falcon-m", head: "FALCON", header: "THE FALCON JAR (THE COIL, CURED)", callouts: ["BEAK: CEREMONIAL, STILL SHARP", "CONTENTS: THE COIL. TRIBUTE, IDEALLY", "LAST WORN AS A HELM, BY SOMETHING BLIND"], footer: "FOOD, TECHNICALLY. TRIBUTE, IDEALLY." });
  });

  /* ---------------- jar-baboon-m (phone twin) ---------------- */
  FIG._define("jar-baboon-m", "svg", function (svg) {
    jarCardM(svg, { key: "jar-baboon-m", head: "BABOON", header: "THE BABOON JAR (THE LUNGS, KEPT)", callouts: ["BROW: DISAPPROVING, PROFESSIONALLY", "CONTENTS: THE LUNGS. STILL AT WORK", "THE GLASS FOGS FROM THE INSIDE"], footer: "FOUR THOUSAND YEARS. STILL COUNTING BREATHS." });
  });

  /* ---------------- jar-human-m (phone twin) ---------------- */
  FIG._define("jar-human-m", "svg", function (svg) {
    jarCardM(svg, { key: "jar-human-m", head: "HUMAN", header: "THE HUMAN JAR (HIS OWN FACE, YOUNG)", callouts: ["THE FACE: HIS OWN, YOUNG. NOT SMILING", "CONTENTS: THE LIVER. GRUDGES INTACT", "THE LIDS: CARVED SHUT. CHECK ANYWAY"], footer: "THE ONLY JAR WITH ITS OWNER'S FACE. IT ISN'T FLATTERED." });
  });

  /* ---------------- centipede-m (phone twin) ---------------- */
  FIG._define("centipede-m", "svg", function (svg) {
    chimCardM(svg, { key: "centipede-m", header: "THE GLASS CENTIPEDE", charred: false, centipede: true, gauge: "VISIBILITY", footer: "FOUR FEET OF GLASS. FASTER THAN THE EYE WANTS TO ALLOW." });
  });

  /* ---------------- chimney-f-m (phone twin) ---------------- */
  FIG._define("chimney-f-m", "svg", function (svg) {
    chimCardM(svg, { key: "chimney-f-m", header: "THE FUNGAL CHIMNEY", charred: false, centipede: false, gauge: null, footer: "SOMETHING LIVED HERE. IT FOLLOWED YOU OUT." });
  });

  /* ---------------- chimney-g-m (phone twin) ---------------- */
  FIG._define("chimney-g-m", "svg", function (svg) {
    chimCardM(svg, { key: "chimney-g-m", header: "THE FUNGAL CHIMNEY, BURNT", charred: true, centipede: true, gauge: "VISIBILITY", footer: "THE GROWTH BURNED. THE GLASS DID NOT." });
  });

  /* ---------------- chimney-h-m (phone twin) ---------------- */
  FIG._define("chimney-h-m", "svg", function (svg) {
    chimCardM(svg, { key: "chimney-h-m", header: "THE FUNGAL CHIMNEY, BURNT", charred: true, centipede: false, gauge: null, footer: "BLACK, BARE, AND BREATHABLE." });
  });

  /* ---------------- autarch-m (phone twin) ---------------- */
  FIG._define("autarch-m", "svg", function (svg) {
    autarchCardM(svg, { key: "autarch-m", header: "NASSAK AN-RAH (AT REST)", state: "rest", footers: ["DREAMING SOMETHING KIND, FOUR THOUSAND YEARS IN."], gauge: null });
  });

  /* ---------------- autarch-c-m (phone twin) ---------------- */
  FIG._define("autarch-c-m", "svg", function (svg) {
    autarchCardM(svg, { key: "autarch-c-m", header: "NASSAK AN-RAH (HOLLOWED)", state: "skull", footers: ["THE STONE WEARS TO THE SKULL. THE SKULL WEARS ORANGE."], gauge: null });
  });

  /* ---------------- autarch-e-m (phone twin) ---------------- */
  FIG._define("autarch-e-m", "svg", function (svg) {
    autarchCardM(svg, { key: "autarch-e-m", header: "THE HORROR, ABLAZE", state: "fire", footers: ["VIGOR", "ABLAZE, IT CANNOT MEND. KEEP CUTTING.", "IT DOES NOT CLOSE AGAIN."], gauge: "VIGOR" });
  });

  /* ---------------- sphere-b-m (phone twin) ---------------- */
  FIG._define("sphere-b-m", "svg", function (svg) {
    sphereCardM(svg, { key: "sphere-b-m", header: "BURIAL SPHERE OF NASSAK AN-RAH", state: "upright", gauge: "THE CHURN", callouts: ["THROAT: THE FUNGUS COMES DOWN IT", "SEAM: VERTICAL, FINE AS A HAIR", "TENANT: COILED TALL, BREATHING SLOW"], footer: "PRY IT, AND IT WILL MIND." });
  });

  /* ---------------- sphere-d-m (phone twin) ---------------- */
  FIG._define("sphere-d-m", "svg", function (svg) {
    sphereCardM(svg, { key: "sphere-d-m", header: "BURIAL SPHERE OF NASSAK AN-RAH", state: "reforged", gauge: "THE KEEPING", callouts: ["FIELD: ANTI-ENTROPY, HOLDING", "SEAM: CLOSED. THE GLASS FORGAVE IT", "TENANT: ASLEEP, DREAMING SOMETHING KIND"], footer: "FOUR THOUSAND YEARS INTO A GOOD NIGHT'S SLEEP." });
  });

  /* ---------------- sphere-e-m (phone twin) ---------------- */
  FIG._define("sphere-e-m", "svg", function (svg) {
    sphereCardM(svg, { key: "sphere-e-m", header: "BURIAL SPHERE OF NASSAK AN-RAH", state: "shatter-vacant", gauge: "THE DRIFT", callouts: ["SHARDS: SIX, FULL-CUT, KEEPING THEIR CURVE", "CRACKS: SIX, ALL TELLING THE SAME STORY", "PRAYERS: ONE OF THEM IS A MENDING"], footer: "THE SHARDS REMEMBER THEIR PLACES." });
  });
  ...
  /* ---------------- spawn-a-m (phone twin) ---------------- */
  FIG._define("spawn-a-m", "svg", function (svg) {
    spawnCardM(svg, { key: "spawn-a-m", header: "SPAWN OF BRAIN (IT HEARD THAT)", state: "listen", footer: "NO EYES UNDER THE JAR. IT DOES NOT NEED THEM." });
  });

  /* ---------------- spawn-b-m (phone twin) ---------------- */
  FIG._define("spawn-b-m", "svg", function (svg) {
    spawnCardM(svg, { key: "spawn-b-m", header: "SPAWN OF BRAIN (MID-STEP)", state: "step", footer: "THE HEAD HOLDS LEVEL. THE REST NEGOTIATES." });
  });

  /* ---------------- spawn-c-m (phone twin) ---------------- */
  FIG._define("spawn-c-m", "svg", function (svg) {
    spawnCardM(svg, { key: "spawn-c-m", header: "SPAWN OF BRAIN (AS FOUND)", state: "felled", footer: "FELLED MID-STEP. THE JAR ROLLED TO ITS SIDE." });
  });

  /* ---------------- guts-a-m (phone twin) ---------------- */
  FIG._define("guts-a-m", "svg", function (svg) {
    gutsCardM(svg, { key: "guts-a-m", header: "SPAWN OF GUTS (SWAYING TOWARD EVERY SOUND)", state: "sway", callouts: ["HELM: THE FALCON JAR. THE PLINTH WANTS IT", "EYES: DECORATIVE. IT HUNTS BY EAR", "COLLAR: GUT, RECENTLY PROMOTED", "ARM: A HOSE WITH OPINIONS"], footer: "WHATEVER YOU JUST DID, IT HEARD." });
  });

  /* ---------------- guts-b-m (phone twin) ---------------- */
  FIG._define("guts-b-m", "svg", function (svg) {
    gutsCardM(svg, { key: "guts-b-m", header: "SPAWN OF GUTS (DETAIL. SORRY.)", state: "drip", callouts: [], footer: "THE EYES ARE ORNAMENTAL. THE HUNGER IS NOT." });
  });

  /* ---------------- guts-c-m (phone twin) ---------------- */
  FIG._define("guts-c-m", "svg", function (svg) {
    gutsCardM(svg, { key: "guts-c-m", header: "SPAWN OF GUTS (THE CLAIM)", state: "claim", callouts: [], footer: "THE JAR IS OWED TO A PLINTH TWO FLOORS UP." });
  });
  ...
  FIG._define("ulfire-m", "svg", function (svg) {
    ulfireCardM(svg, { key: "ulfire-m", header: "THE ULFIRE LANTERN", lit: true, callouts: ["ROOT: THE COFFIN", "SUMMIT: THE MYSTIC", "WHERE IT SHINES, STONE FORGETS TO BE SOLID"], footer: "LEAD WOULD STOP IT. THE TOMB DID NOT THINK OF THAT." });
  });
