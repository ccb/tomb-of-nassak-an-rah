...
(() => {
  ...
  /* ---------------- 11-C: the burial sphere, shattered ---------------- */
  FIG._define("sphere-c", "svg", function (svg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 420, fill: BG });
    el(svg, "line", { x1: 16, y1: 34, x2: 624, y2: 34, stroke: PH_DIM });
    const hdr = label(svg, 16, 24, 13, PH_BRIGHT);
    const cls = label(svg, 624, 24, 10, PH_DIM); cls.setAttribute("text-anchor", "end");
    ...
    const six = label(svg, 424, 300, 64, PH_DIM);
    six.setAttribute("opacity", .3); six.textContent = "6";
    // the ghost of the oval: where the pieces used to agree
    el(svg, "ellipse", { cx: 320, cy: 232, rx: 30, ry: 52, fill: "none",
      stroke: PH_DIM, "stroke-dasharray": "2 5", opacity: .5 });
    // six arcs of the broken egg, each still keeping its curve
    const SHARDS = Array.from({ length: 6 }, (_, i) => ({
      n: el(svg, "polyline", { fill: "none", stroke: PH_BRIGHT, "stroke-width": 1.8 }),
      a0: i * Math.PI / 3 + .1, a1: (i + 1) * Math.PI / 3 - .1,
    }));
    const motes = Array.from({ length: 8 }, (_, i) =>
      ({ q: el(svg, "circle", { r: i % 3 ? 1 : 1.6, fill: PH_DIM }),
         R: 44 + (i * 29) % 68, ph: i * .8, sp: .04 + (i % 3) * .015 }));
    ...
    const CALLS = [
      ...
      { at: [352, 198], text: "SHARDS: SIX, KEEPING THEIR CURVE" },
      { at: [320, 180], text: "SEAM: WHERE IT LET GO" },
    ...
    clock(t => {
      ...
      SHARDS.forEach((s, i) => {
        ...
        const d = 20 + 7 * Math.sin(t * .05 + i * 1.9);       // out, and back a little
        const spin = Math.sin(t * .03 + i * 2.4) * .5;        // a lazy tumble
        const ox = Math.cos(mid) * d * .9, oy = Math.sin(mid) * d;
        const cx0 = 320 + Math.cos(mid) * 30 * .62, cy0 = 232 + Math.sin(mid) * 52 * .62;
        const pts = [];
        for (let k = 0; k <= 6; k++) {
          const a = s.a0 + (s.a1 - s.a0) * k / 6;
          let px = 320 + Math.cos(a) * 30, py = 232 + Math.sin(a) * 52;
          const rx0 = px - cx0, ry0 = py - cy0;               // tumble about its middle
          px = cx0 + rx0 * Math.cos(spin) - ry0 * Math.sin(spin);
          py = cy0 + rx0 * Math.sin(spin) + ry0 * Math.cos(spin);
          pts.push((px + ox).toFixed(1) + "," + (py + oy).toFixed(1));
        }
