...
(() => {
  "use strict";
  const META = {"autarch": ["svg", 640, 420], "autarch-c": ["svg", 640, 420], "autarch-e": ["svg", 640, 420], "bats": ["canvas", 640, 300], "bats-c": ["svg", 640, 360], "blade": ["canvas", 640, 360], "canopic-c": ["svg", 640, 400], "centipede": ["svg", 640, 360], "cylinders": ["svg", 640, 400], "cylinders-b": ["svg", 640, 400], "epitaph": ["svg", 640, 400], "ext1c": ["svg", 640, 400], "ext1e": ["svg", 640, 400], "fungus": ["svg", 640, 360], "glowstone": ["canvas", 640, 300], "guts-a": ["svg", 640, 360], "guts-b": ["svg", 640, 360], "guts-c": ["svg", 640, 360], "hound": ["svg", 640, 360], "jackal": ["svg", 640, 360], "jar-falcon": ["svg", 640, 300], "jar-jackal": ["svg", 640, 300], "jar-mantis": ["svg", 640, 300], "mystic-b": ["svg", 640, 400], "road": ["svg", 640, 400], "seal": ["svg", 640, 400], "seal-a": ["svg", 640, 400], "seal-b": ["svg", 640, 400], "silas": ["svg", 640, 400], "spawn-a": ["svg", 640, 360], "spawn-b": ["svg", 640, 360], "spawn-c": ["svg", 640, 360], "sphere-b": ["svg", 640, 420], "tesseract": ["canvas", 640, 360]};
  const FIG = {
  ...
  /* ------- 18-A: the seal answers the jars (first state; archival) ------- */
  FIG._define("seal-a", "svg", function (svg) {
    el(svg, "rect", { x: 0, y: 0, width: 640, height: 400, fill: BG });
    el(svg, "line", { x1: 16, y1: 34, x2: 624, y2: 34, stroke: PH_DIM });
    const hdr = label(svg, 16, 24, 13, PH_BRIGHT);
    const cls = label(svg, 624, 24, 10, PH_DIM); cls.setAttribute("text-anchor", "end");
    cls.textContent = "HALL OF THE CANOPIC JARS / FIRST STATE";
    const JDOTS = stipple(svg, "dots-seal-a", PH, .9);
    const CRIM = "#ff3b57", WHITE = "#eaf6ff";
    const arch = el(svg, "path", { fill: "none", stroke: PH_DIM, "stroke-width": 1.8,
      d: "M 246 316 L 246 170 Q 320 108 394 170 L 394 316" });
    // the crimson gate, barred until the hall is fully attended
    ...
    [["BABOON", 0], ["MANTIS", 2], ["JACKAL", 4]].forEach(([kind, i]) =>
      jar(kind).setAttribute("transform", `translate(${PLINTHS[i]} -29)`));
    const m1 = jar("HUMAN"), m2 = jar("FALCON");
  ...
  FIG._define("centipede", "svg", function (svg) {
    ...
    const adv = T => DASH.reduce((a, [s2, e2]) =>
      a + Math.max(0, Math.min(T, e2) - s2) * 14, 0);
    ...
    const glints = ["#ff5a5a", "#ffd76a", "#7fff9e", GLASS].map(c =>
      el(svg, "circle", { r: 1.5, fill: c, opacity: 0 }));
    ...
    clock(t => {
      ...
      glints.forEach((n, j) => {                              // stray refraction, between
        const gi = (Math.floor(t / 2) * 7 + j * 5) % 16;
        const x = hx - gi * 13;
        n.setAttribute("cx", x + ((t + j) % 3) - 1); n.setAttribute("cy", wave(x) + ((t * j) % 5) - 2);
        n.setAttribute("opacity", !seen && (t + j * 2) % 5 < 1 && x > 0 && x < 640 ? .9 : 0);
      });
